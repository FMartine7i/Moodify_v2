...
const getSongs = async (req, res) => {
    try {
        ...
        const query = {};
        if (album)
            query.album = { $regex: album, $options: 'i' }; // búsqueda insensible a mayúsculas y minúsculas
        if (title)
...
const getSongsByMood = async (req, res) => {
    try {
        ...
        if (songsFromDB.length >= 10)
            return res.status(200).json({ local: songsFromDB, spotify: [] });
