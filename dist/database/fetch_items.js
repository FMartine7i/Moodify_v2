...
const fetchAndSaveSongs = async () => {
    try {
        ...
        if (!playlist_id)
            throw new Error('La variable PLAYLIST_ID no está definida en el archivo .env');
...
const fetchAndSavePlaylists = async () => {
    try {
        ...
        for (const playlist of playlists) {
            ...
            await Playlist.updateOne({ name: playlist.name, image: playlist.images[0].url }, { $set: newPlaylist }, { upsert: true } // si el álbum no está en la BD, lo crea
            );
        }
